...
$backToWeatherFromFavorites.addEventListener('click', (event: Event) => {
  event.preventDefault();
  swapView('weather-app');
});
...
function getWeatherIcon(weatherCondition: string): string {
  switch (weatherCondition) {
    case 'clear':
...
$favoritesButton2.addEventListener('click', (event: Event) => {
  event.preventDefault();
  swapView('favorite-locations');
});
$backToWeatherFromFavorites.addEventListener('click', (event: Event) => {
